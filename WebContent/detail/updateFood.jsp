                    ...
                    <tr>
                            ...
                            <select name=typeid style="width:80px">
	                            <jsp:useBean id="ml" class="com.et.model.food.MyFood"></jsp:useBean>
			   						<c:forEach var="v" items="${ml.foodType}">
				   						<option value="${pageScope.v.TYPEID}" 
				   						<c:if test="${pageScope.v.TYPEID==requestScope.foodlist[0].TYPEID}">selected="selected"</c:if>
				   							
				   						>${pageScope.v.TYPENAME}</option>
			   						</c:forEach>
                            </select>
                             *<input type="hidden" name="foodid" value="${requestScope.foodlist[0].FOODID}" /></td>
						</tr>
						<tr>
							<td width="80px">菜名</td>
							<td><input type="text" name="foodName" class="InputStyle" value="${requestScope.foodlist[0].FOODNAME}"/> *</td>
						</tr>
						<tr>
							<td>价格</td>
							<td><input type="text" name="price" class="InputStyle" value="${requestScope.foodlist[0].PRICE}"/> *</td>
						</tr>
						
						<tr>
							<td>简介</td>
							<td><textarea name="introduce" class="TextareaStyle">${requestScope.foodlist[0].DESCE}</textarea></td>
						</tr>
						<tr>
							<td width="80px">菜品图片</td>
							<td>
									<input type="hidden" name="image" value="${foodlist[0].IMAGEURL}">
									<img style='max-width:68px;width:68px;width:expression(width>68?"68px":width "px");max-width: 68px;' 
									src="${pageContext.request.contextPath}/ShowImageServlet?imageName=${foodlist[0].IMAGEURL}">
